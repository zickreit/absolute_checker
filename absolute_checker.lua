...
function check_for_updates(manual)
    ...
    github_request(version_url, function(content, code)
        if code == 200 and content then
            local remote_version = content:match("%S+")  -- берём первую строку без пробелов
            if remote_version and remote_version ~= CURRENT_VERSION then
                ...
                -- сохраняем информацию о новой версии для подтверждения
            ...
            if manual then sampAddChatMessage("[AdminChecker] Не удалось проверить обновления. Код ошибки: " .. tostring(code), 0xFF0000) end
        end
    end)
end
